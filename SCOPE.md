...
- **Rule-based signal:** heuristic detection of peel chains, rapid fan-out, and mixer-adjacent hops. **Explicit Boundary:** Because the Elliptic dataset completely anonymizes node IDs as integers, there is no possible linkage to real Bitcoin mixer addresses. Therefore, the `mixer_adjacent` rule *cannot* be evaluated or tuned on historical training data. It will only be exercised on live-traced wallets (Phase R7). **Current status (2026-09-06 audit, investigation complete):** 2 of the 3 heuristics are active — `mixer_adjacent` is implemented but **dormant**. `data/mixers.txt` is intentionally empty because a genuine investigation was conducted and no citable, verifiable public source of exact Bitcoin mixer addresses was found (Europol/Chainalysis ChipMixer reporting and the academic tumbler literature were checked; they describe services but publish no exact addresses; academic datasets exist but were not accessible to verify entry-by-entry). Per the sourcing rule (never fabricate), the heuristic stays silent — its rule_score contribution is simply never earned. This is not an open TODO; it is a completed investigation with a "no" answer.
...
- **Clustering:** grouping wallets likely controlled by the same entity, using Elliptic-derived data as the primary (higher-confidence) source. **Current implementation note:** common-input and change-address clustering apply to live-traced data only, because the Elliptic++ actor dataset is not available. **Wiring status (2026-09-06):** live-UTXO clustering is now exposed via `GET /clusters/live?address=<addr>`, which calls `BlockstreamClient.iter_address_internal_txs()` → `build_live_clusters()` and returns `TIER_LIVE_UTXO` clusters. Real cost tradeoff: up to `LIVE_MAX_TXS_PER_ADDRESS` (50) additional `GET /tx/{txid}` calls per request, beyond the cheaper tx-summary calls used by `/trace`. Acceptable for single-wallet demo use per SCOPE.md; not supported for bulk tracing (already out of scope). The `nx.DiGraph` used by `/trace`/`/verdict` and the `nx.Graph` used for clustering are separate paths and are never merged.
...
- **Sourcing a Bitcoin mixer-address validation set.** A genuine investigation was completed (2026-09-06) and no citable, verifiable public source of exact mixer addresses was found (Europol/Chainalysis ChipMixer reporting and academic tumbler literature checked; no per-address data accessible). The `mixer_adjacent` heuristic remains dormant by design — it never fires on made-up addresses. This is a documented, closed investigation, not an open TODO.
